...
export function getCoverageAges(taxonomy) {
  return taxonomy.ageGroups.filter(age => age.coverageTarget !== false);
}

function expandedCoverageAgeIds(activity, taxonomy) {
  const ids = new Set(activity.ageGroups || []);
  if (ids.has("all-ages")) {
    for (const age of getCoverageAges(taxonomy)) ids.add(age.id);
  }
  return [...ids];
}
...
export function buildCoverageMatrix(activities, taxonomy, config = DEFAULT_COVERAGE_CONFIG) {
  const matrix = {};
  const coverageAges = getCoverageAges(taxonomy);
  const priorityAreas = new Set(taxonomy.areas.filter(area => area.priorityDepth).map(area => area.id));

  for (const area of taxonomy.areas) {
    matrix[area.id] = {};
    for (const age of coverageAges) {
      matrix[area.id][age.id] = { count: 0, status: "red", activityIds: [] };
    }
  }
  ...
  for (const activity of activities) {
    ...
    for (const areaId of activity.areas || []) {
      ...
      for (const ageId of expandedCoverageAgeIds(activity, taxonomy)) {
  ...
  for (const area of taxonomy.areas) {
    const target = priorityAreas.has(area.id) ? config.priorityHealthyMinimum : config.healthyMinimum;
    for (const age of coverageAges) {
      const cell = matrix[area.id][age.id];
      cell.status = cell.count === 0 ? "red" : cell.count < target ? "amber" : "green";
      cell.target = target;
...
export function candidateCoverageValue(candidate, matrix, taxonomy) {
  let score = 0;
  const fills = [];
  const ageIds = taxonomy ? expandedCoverageAgeIds(candidate, taxonomy) : (candidate.ageGroups || []);
  for (const areaId of candidate.areas || []) {
    for (const ageId of ageIds) {
      ...
      score += value;
      if (value > 0) fills.push({ areaId, ageId, currentCount: cell.count, target: cell.target, value });
    }
  }
  return { score, fills };
}
